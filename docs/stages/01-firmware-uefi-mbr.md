...
| **<Term term="LONG_MODE">Long Mode</Term>** (64-битный x64) | 64 бит | **До 256 Тбайт** (48/57 бит) | ✅ **Кольца Ring 0/3**, NX-бит (защита от выполнения), плоская память. | Основной режим работы 64-битной Windows NT, DXE/BDS фазы UEFI. |
...
3. Регистр указателя стека `ESP / RSP` указывает на этот кэшированный регион, позволяя прошивке безопасно исполнять сложный C/C++ код задолго до подачи питания на планки DRAM.
